...
public class WeightsConfig {
	...
	private static void loadConfig() {
		...
		for (Item item : itemList) {
			config.getFloat(item.getTranslationKey().substring(5), GENERAL, generateWeight(item), 0, 16,
					"Sets the carry weight of item " + item.getTranslationKey().substring(5, item.getTranslationKey()
							.length()));
		}

		List<Block> blockList = ImmutableList.copyOf(Block.REGISTRY);
		for (Block block : blockList) {
			config.getFloat(block.getTranslationKey().substring(5, block.getTranslationKey().length()), GENERAL,
					generateWeight(block), 0, 16, "Sets the carry weight of block " + block.getTranslationKey()
							.substring(5, block.getTranslationKey().length()));
		}
