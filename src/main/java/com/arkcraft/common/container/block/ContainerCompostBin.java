	...
	public ContainerCompostBin(EntityPlayer player, TileEntityCompostBin tileEntity) {
		...
		for (int row = 0; row < 3; row++)
			for (int col = 0; col < 9; col++)
				addSlotToContainer(new Slot(playerInventory, 9 + col + row * 9, playerXStart + col * 18, playerYStart + row * 18));
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (slot.inventory == inventory) {
				if (!this.mergeItemStack(itemstack1, 8, 8 + 36, false))
					return null;
			} else if (slot.inventory == playerInventory)
		...
		for (int i = 0; i < this.listeners.size(); ++i) {
			...
			for (int fieldID = 0; fieldID < inventory.getFieldCount(); ++fieldID) {
				if (fieldHasChanged[fieldID]) {
					// Note that although sendWindowProperty takes 2 ints on
