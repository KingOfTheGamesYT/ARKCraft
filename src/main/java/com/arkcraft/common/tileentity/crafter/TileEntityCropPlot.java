...
public class TileEntityCropPlot extends TileEntityArkCraft implements IInventory, IHoverInfo, IDecayer, ITickable {
	...
	public ItemStack decrStackSize(int slot, int par2) {
		...
		if (te.stack[slot] != null) {
			ItemStack itemstack;
			if (te.stack[slot].getCount() <= par2) {
		...
		if (!world.isRemote) {
			...
			if (part == Part.MIDDLE) {
				...
				if (growing == null && fIndex > -1 && sIndex > -1 && water.getFluidAmount() > 0) {
					growthTime = ModuleItemBalance.CROP_PLOT.SEEDLING_TIME_FOR_BERRY * 20;
					if (LOG)
					...
					if (growthTime >= 0 && world.getLight(pos) > 7) {
						if (fIndex > -1 && water.getFluidAmount() > 0) {
							...
							if (rand % (world.getDifficulty() == EnumDifficulty.NORMAL ? 40 : (world.getDifficulty() == EnumDifficulty.EASY ? 30 : 20)) == 0 && world.getDifficulty() != EnumDifficulty.HARD) {
								ret = TileEntityHopper.putStackInInventoryAllSlots(this, this, growing, null) == null;
							}
							if (LOG)
					...
					if (growthTime < 0) {
						if (this.state == CropPlotState.FRUITLING) {
							...
							if (success) {
								...
								TileEntityHopper.putStackInInventoryAllSlots(this, this, r, null);
							} else {
								if (LOG)
									LogHelper.info("[Crop Plot at " + pos.getX() + ", " + pos.getY() + ", " + pos.getZ() + "]: Growing Failed: " + growing);
							}
							growthTime = MathHelper.floor(ModuleItemBalance.CROP_PLOT.FRUIT_OUTPUT_TIME_FOR_BERRY * (20D * ((world.getDifficulty().ordinal() * 0.5D) + 1)));
						} else {
	...
	public void addInformation(List<String> text) {
		String name = I18n.translate(growing != null ? growing.getTranslationKey() + ".name" : "arkcraft.empty");
		...
		for (int i = 0; i < 10; i++) {
			if (stack[i] != null) {
				...
				if (item instanceof ItemFertilizer) {
					f += ItemFertilizer.getFertilizingValueLeft(stack[i]) * stack[i].getCount();
				}
			}
		}
		String toAdd = "" + f / 20;
		...
		if (growthTime > 0)
			text.add(TextFormatting.GRAY + I18n.format("arkcraft.cropPlotGrowth", Utils.formatTime(growthTime).substring(1)));
	}
	...
	public BlockCropPlot.BerryColor getGrowingColor() {
		return growing != null && growing.getItem() instanceof ARKCraftSeed ? ((ARKCraftSeed) growing.getItem()).getBerryColor(growing) : BlockCropPlot.BerryColor.AMAR;
	}
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		if (!isTransparent()) {
			...
			growing = new ItemStack(compound.getCompoundTag("growing"));
		}
	}
	...
	public static enum CropPlotState {
		...
		SEEDLING(1) {
			@Override
			public int getTime() {
				return ModuleItemBalance.CROP_PLOT.SEEDLING_TIME_FOR_BERRY;
			}
		},
		MIDLING(2) {
			@Override
			public int getTime() {
				return ModuleItemBalance.CROP_PLOT.MIDLING_TIME_FOR_BERRY;
			}
		},
		GROWTHLING(3) {
			@Override
			public int getTime() {
				return ModuleItemBalance.CROP_PLOT.GROWTHING_TIME_FOR_BERRY;
			}
		},
		FRUITLING(4) {
			@Override
