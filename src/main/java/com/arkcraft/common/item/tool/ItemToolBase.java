...
import com.arkcraft.common.item.IBreakable;
...
public abstract class ItemToolBase extends ItemQualitable implements IBreakable {
	...
	@Override
	public final float getDestroySpeed(ItemStack itemstack, IBlockState state) {
		...
		if (entityLiving instanceof EntityPlayer && !entityLiving.isSwingInProgress) {
			...
			if (mop.typeOfHit == RayTraceResult.Type.BLOCK) {
				...
				if (target != null) {
					...
					int size = bs.getBlock() == Blocks.LOG || bs.getBlock() == Blocks.LOG2 ? countTree(w, pos, target) : countOre(w, pos, target, entityLiving, stack, false);
