...
public class BlockSmithy extends BlockARKContainer {
	...
	@Override
	public boolean onBlockActivated(World worldIn, BlockPos blockPos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (!playerIn.isSneaking()) {
			if (state.getValue(PART) == EnumPart.RIGHT)
				...
				oState.getBlock().onBlockActivated(worldIn, pos, oState, playerIn, hand, side, hitX, hitY, hitZ);
			}
			return true;
		}
		return false;
...
//TODO refactor to account for new implementation
		if (observerState.getValue(PART) == EnumPart.LEFT) {
