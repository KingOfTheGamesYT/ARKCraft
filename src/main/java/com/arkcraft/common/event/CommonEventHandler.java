				...
				if (bookSpawnDelay > 0) bookSpawnDelay--;
					...
					for (Entity entityInWorld : entitiesInWorld) {
						...
						if (entityInWorld instanceof EntityItem) {
							...
							if (entityItemInWorld.getItem().getItem() == Items.BOOK) {
								...
								for (Entity entityWithinBound : entitiesWithinBound) {
									if (entityWithinBound instanceof EntityItem) {
										...
										if (entityItemWithinBound.getItem().getItem() == Items.BONE) {
										...
										} else if (entityItemWithinBound.getItem().getItem() == Items.WHEAT) {
										...
										if (remainingInputs.isEmpty()) {
											...
											for (EntityItem foundEntityItem : foundEntityItems) {
												...
												foundEntityItem.getItem().shrink(1);
												if (foundEntityItem.getItem().getCount() <= 0) {
													LogHelper.info("Deleting the Item: " + foundEntityItem
															.getItem().getItem().toString());
				...
				if (event.phase == Phase.START) {
					...
					if (t.tick > 20) {
						...
						for (int i = 0; i < event.world.loadedTileEntityList.size(); i++) {
							if (event.world.loadedTileEntityList.get(i) instanceof IInventory) {// Check
								// for
								// inventories
								// every
								// second
				...
				if (in instanceof IDecayable) {
					...
					for (int i = 0; i < p.inventory.getSizeInventory(); i++) {
						...
						if (inInv != null && inInv.getItem() == in) {
							...
							int inSize = inInv.getCount();
							int pickSize = pickedUp.getCount();
