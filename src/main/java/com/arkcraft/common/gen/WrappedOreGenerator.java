...
public class WrappedOreGenerator implements IWorldGenerator {
	...
	public WrappedOreGenerator(int par1, Instruction... par2) {
		...
		for (Instruction var : par2) {
			if (var.getBlocksPerVein() > 0 && var.getVeinsPerChunk() > 0 && var.getMaxHeight() > 0) {
				generations.put(var.getBlockType().getTranslationKey(), var);
			}
		}
	}
	...
	public static class Instruction {
		 ...
		 * @param par2 the maximum height the blocks will appear on (64 is
		 *             surface).
		 * @param par3 the maximum amount of blocks that can be generated in a
		 *             vein.
		 ...
		 *
		 * @param par1 an {@link #Integer} array representing the dimension id's
		 *             to run.
