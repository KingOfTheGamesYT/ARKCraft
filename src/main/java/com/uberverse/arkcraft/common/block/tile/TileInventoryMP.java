...
public class TileInventoryMP extends TileEntity implements IInventory, IUpdatePlayerListBox
{
	...
	private boolean craftItem(boolean doCraftItem)
	{
		...
		// LogHelper.info("TileInventoryMP: Update called on " +
		// (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT ?
		// "client" : "server"));
		// LogHelper.info("TileInventoryMP: craftAll = " + (craftAll == true ?
		// "true" : "false"));
	...
	// entity unloads
	// In this case, it saves the state of the mortar and pestle and the
	// itemstacks stored in the inventory
	@Override
