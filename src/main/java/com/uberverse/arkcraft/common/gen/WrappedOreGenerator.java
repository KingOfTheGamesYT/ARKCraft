...
public class WrappedOreGenerator implements IWorldGenerator
{
	...
	public static class Instruction
	{
		 ...
		 *
		 * @param par1
		 *            the block type to use when generating.
		 * @param par2
		 *            the maximum height the blocks will appear on (64 is
		 *            surface).
		 * @param par3
		 *            the maximum amount of blocks that can be generated in a
		 *            vein.
		 * @param par4
		...
		/**
		 * Set whether this instruction should be ran when the world generation
		 * starts.<br>
		 * Set to false if you want the instruction to be ignored by the
		 * wrapper.
		 *
		 * @param par1
		 *            the value.
		...
		/**
		 * Returns the dimensions to run this instruction on. Just set to
		 * <b>null</b>.<br>
		 ...
		 *
		 * @param par1
		 *            an {@link #Integer} array representing the dimension id's
		 *            to run.
	...
	private void generateInDimension(Instruction par1, World world, Random random, int x, int z)
	{
		for (int k = 0; k < par1.getVeinsPerChunk(); k++)
		{
			...
			new WorldGenMinable(par1.getBlockType().getDefaultState(), par1.getBlocksPerVein())
