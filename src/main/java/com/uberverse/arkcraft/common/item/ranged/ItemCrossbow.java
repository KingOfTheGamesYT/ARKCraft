...
public class ItemCrossbow extends ItemRangedWeapon
{
	...
	{
		world.playSoundEffect(x, y, z, "random.bow", 1.0F, 1.0F / (this.getItemRand().nextFloat() * 0.4F + 0.8F));
	}

	@Override
	public void effectReloadDone(ItemStack stack, World world, EntityPlayer player)
	{
		world.playSoundAtEntity(player, "random.click", 0.8F, 1.0F / (this.getItemRand().nextFloat() * 0.4F + 0.4F));
	}
}
