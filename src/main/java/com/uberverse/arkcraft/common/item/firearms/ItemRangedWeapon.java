...
public abstract class ItemRangedWeapon extends ItemBow {
	...
	public ItemRangedWeapon(String name, int durability, int maxAmmo, String defaultAmmoType, int ammoConsumption,
			double shotInterval, float speed, float inaccuracy) {
		super();
		this.speed = speed;
		...
		if (mop != null) {
			if (!(mop.typeOfHit == MovingObjectPosition.MovingObjectType.MISS)) {
				...
				if (entityIn.worldObj.getBlockState(pos).getBlock() == ARKCraftBlocks.block_flashlight) {
					TileFlashlight tileLight = (TileFlashlight) entityIn.worldObj.getTileEntity(pos);
					tileLight.ticks = 0;
				} else if (entityIn.worldObj.isAirBlock(pos)) {
					entityIn.worldObj.setBlockState(pos, ARKCraftBlocks.block_flashlight.getDefaultState());
				}
			}
		}
	}
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
		if (stack.stackSize <= 0 || player.isUsingItem()) {
			return stack;
		}
	...
	protected EntityProjectile createProjectile(ItemStack stack, World world, EntityPlayer player) {
		try {
			...
			Class<?> c = Class.forName(
					"com.uberverse.arkcraft.common.entity." + ProjectileType.valueOf(type.toUpperCase()).getEntity());
