...
public abstract class ARKCraftTool extends ItemTool
{
	...
	private static final Predicate<IBlockState> WOOD_PREDICATE =
			new Predicate<IBlockState>()
			{
				...
				{
					return blockState.getBlock() == Blocks.log
							|| blockState.getBlock() == Blocks.log2;
				}
			};
	private static final Predicate<IBlockState> IRON_ORE_PREDICATE =
			new Predicate<IBlockState>()
			{
	...
	// For the ItemDrop
	private void entityDropItem(World worldIn, BlockPos pos, Block block,
			...
			Block blockIn, BlockPos pos, EntityLivingBase playerIn)
	{
				...
				&& ARKPlayer.isARKMode((EntityPlayer) playerIn))
		{
			...
			if (WOOD_PREDICATE.apply(blockState))
			{
				...
				int thatch =
						calcOutput(count, toolType.getPickaxeModifier(), 1);
				entityDropItem(worldIn, pos, blockIn, player,
						new ItemStack(ARKCraftItems.wood, wood));// (int)
																	// (10
																	// +
																	// itemRand.nextInt(100)/20.0*count*toolType.getHatchetModifier())
				entityDropItem(worldIn, pos, blockIn, player,
						new ItemStack(ARKCraftItems.thatch, thatch));// (int)
																		// (10
																		// +
			...
			else if (blockState.getBlock() == Blocks.stone)
			{
				...
				EnumFacing f = Utils.getDirectionFacing(playerIn, false);
				{
					...
					{
						IBlockState blockState2 =
								worldIn.getBlockState(pos2.down());
						if (blockState2.getBlock() == Blocks.stone)
						{
							multiplier++;
				...
				int stone = calcOutput(multiplier,
						toolType.getPickaxeModifier(), 1);
				int flint = calcOutput(multiplier,
						toolType.getHatchetModifier(), 1);
				int metal = calcOutput(multiplier,
						toolType.getPickaxeModifier(), 0.1D);
				entityDropItem(worldIn, pos, blockIn, player,
						new ItemStack(ARKCraftItems.stone, stone));// (int)
																	// (10
																	// +
																	// itemRand.nextInt(100)/20.0*multiplier*toolType.getPickaxeModifier())
				entityDropItem(worldIn, pos, blockIn, player,
						new ItemStack(ARKCraftItems.flint, flint));// (int)
																	// (10
																	// +
																	// itemRand.nextInt(100)/20.0*multiplier*toolType.getHatchetModifier())
				entityDropItem(worldIn, pos, blockIn, player,
						new ItemStack(ARKCraftItems.metal, metal));// (int)
																	// (1
																	// +
																	// itemRand.nextInt(100)/20.0*multiplier*toolType.getPickaxeModifier())
			}
			else if (IRON_ORE_PREDICATE.apply(blockState))
			{
				...
				int stone =
						calcOutput(count, toolType.getPickaxeModifier(), 0.8D);
	...
	private static int calcOutput(int count, float toolModifier,
	...
	public void registerModels()
	{
		...
		{
			p.registerItemTexture(this, i,
					"tool_" + getUnlocalizedName().substring(5) + "_"
			...
			List tooltip, boolean advanced)
	{
		...
		tooltip.add(I18n.format("arkcraft.tooltip.toolLevel",
				ToolLevel.VALUES[stack.getMetadata() % ToolLevel.VALUES.length]
						.getTranslatedName()));
		if (advanced)
		{
			int max = ToolLevel.VALUES[stack.getMetadata()
					% ToolLevel.VALUES.length]
							.getDurrability(toolMaterial.getMaxUses());
			tooltip.add(
					"Durability: " + max + "/" + (max - getToolDamage(stack)));
		}
	}
	...
	public boolean damageTool(ItemStack toolStack, EntityLivingBase entityIn)
	{
		...
		if (newValue >= getDurability(toolStack))
		{
			...
			if (entityIn instanceof EntityPlayer)
			{
				...
				entityplayer.triggerAchievement(StatList.objectBreakStats[Item
						.getIdFromItem(toolStack.getItem())]);
