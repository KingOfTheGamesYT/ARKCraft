...
public abstract class ARKCraftTool extends ItemTool
{
	...
	private void entityDropItem(World worldIn, BlockPos pos, Block block, EntityPlayer playerIn, ItemStack itemStackIn)
	{
		if (itemStackIn.stackSize != 0 && itemStackIn.getItem() != null)
		{
			...
			EntityItem entityitem = new EntityItem(worldIn, pos.getX() + offset,
					pos.getY() + block.getBlockBoundsMaxY(), pos.getZ() + offset, itemStackIn);
	...
	{// TODO: Call this from the Client proxy for each tool item.
		...
		for (int i = 0; i < list.size(); i++)
		{
			p.registerItemTexture(this, i,
					"tool_" + getUnlocalizedName().substring(5) + "_" + ToolLevel.VALUES[i].name);
		}
	}
	...
	public void addInformation(ItemStack stack, EntityPlayer playerIn, List tooltip, boolean advanced)
	{
		...
		if (advanced)
		{
			int max = ToolLevel.VALUES[stack.getMetadata() % ToolLevel.VALUES.length]
		...
		if (newValue >= getDurability(toolStack))
		{
			...
			if (entityIn instanceof EntityPlayer)
			{
				...
				entityplayer.triggerAchievement(
						StatList.objectBreakStats[Item.getIdFromItem(toolStack.getItem())]);
