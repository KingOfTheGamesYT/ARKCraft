...
public class ARKShapelessRecipe implements IARKRecipe
{
	...
	/**
	 * Craft one item Returns - 1 if item was crafted, 0 if not
	 */
	...
	public int craftMatches(ItemStack[] itemStacksInventory)
	{
		...
		for (int i = 0; i < itemStacksInventory.length; ++i)
		{
			...
			if (itemstack != null)
			{
				...
				if (recipelist.isEmpty()) { return 1; }
			}
		}
		return 0;
	}
	...
	/**
	 * Used to check if a recipe matches current crafting inventory Only call
	 * with craft = true after ensuring enough inventory with a previous call
	 * with it set to false Returns - number of items that can be crafted
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
