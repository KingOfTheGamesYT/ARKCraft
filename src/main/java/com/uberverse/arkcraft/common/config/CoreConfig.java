...
public class CoreConfig
{
	...
	{
		// Load all the Categories (a function for each just below this
		// function)
	...
	public static void init(File configDir)
	{
		...
		File configFile = new File(configDir, ARKCraft.MODID + "_mod1.cfg");
		if (config == null)
		{
	...
	/**
	 * This method makes variables from a class available in the config file. To
	 * use this, the given class can only contain static non-final variables,
		...
		for (Field f : cls.getDeclaredFields())
		{
			...
			try
			{
				if (type == int.class)
				{
					...
					f.set(null, config.get(cat.getQualifiedName(), a.name(), a.value(), a.comment(), a.minValue(), a
							.maxValue()).getInt());
				}
			...
			{
				LogHelper.error("Configs: Author probably forgot to specify a default value for " + name + " in " + cls
						.getCanonicalName() + e1);
