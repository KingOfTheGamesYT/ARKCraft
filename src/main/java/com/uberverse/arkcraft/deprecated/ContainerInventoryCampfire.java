...
public class ContainerInventoryCampfire extends Container
		implements IBurnerContainer
{

	// Stores the tile entity instance for later use
	private TileInventoryCampfire tileInventoryCampfire;
			...
			int sourceSlotIndex)
	{
		...
		else if (sourceSlotIndex >= FIRST_CAMPFIRE_SLOT_INDEX
				&& sourceSlotIndex < FIRST_CAMPFIRE_SLOT_INDEX
						+ CAMPFIRE_SLOT_COUNT)
		{
	...
	public void detectAndSendChanges()
	{
		...
		boolean fieldHasChanged[] =
				new boolean[tileInventoryCampfire.getFieldCount()];
		if (cachedFields == null)
		{
			cachedFields = new int[tileInventoryCampfire.getFieldCount()];
			allFieldsHaveChanged = true;
		}
		for (int i = 0; i < cachedFields.length; ++i)
		{
			if (allFieldsHaveChanged
					|| cachedFields[i] != tileInventoryCampfire.getField(i))
			{
				cachedFields[i] = tileInventoryCampfire.getField(i);
		...
		for (int i = 0; i < this.crafters.size(); ++i)
		{
			...
			for (int fieldID = 0;
					fieldID < tileInventoryCampfire.getFieldCount(); ++fieldID)
			{
