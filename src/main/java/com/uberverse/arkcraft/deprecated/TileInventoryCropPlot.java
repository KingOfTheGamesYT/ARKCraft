...
public class TileInventoryCropPlot extends TileEntity implements IInventory, IUpdatePlayerListBox
{
	 ...
	 */
	private short MAXIMUM_WATER_TIME = (short) (waterTimeInitialValue * 5); // maximum
																			// is
																			// 32,767
																			// for
																			// a
	...
	/**
	 * Returns the amount of fertilizer remaining on the currently burning item
	 * in the given fertilizer slot.
	 *
	 * @param fertilizerSlot
	 *            the number of the fertilizer slot (0..5)
	 * @return fraction remaining, between 0 - 1
	 */
	public double fractionOfFertilizerRemaining(int fertilizerSlot)
	{
		if (itemStacks[fertilizerSlot] == null) { return 0.0D; }
		double fraction = secondsOfFertilizerRemaining(fertilizerSlot) / itemStacks[fertilizerSlot]
				.getMaxDamage();
	...
	public int secondsOfFertilizerRemaining(int fertilizerSlot)
	{
		...
		{
			return itemStacks[fertilizerSlot].getMaxDamage() - itemStacks[fertilizerSlot]
					.getItemDamage();
		}
		else
		{
	 ...
	 *
	 * @return number of slots with burning fertilizer, 0 -
	 *         FERTILIZER_SLOTS_COUNT
	 */
	...
	// - see if the fertilizer has run out, and if so turn the crop plot "off"
	// and slowly un-grow the current item (if any)
	...
	public void update()
	{
		...
		// If there is no seed or water, or there is no room in the output,
		// reset growTime and return
		if (canHarvest())
		{
			int numberOfFertilizerBurning = burnFertilizer();

			// If fertilizer is available, keep growing the item, otherwise
			// start "un-growing" it at double speed
			...
			// If growTime has reached maximum, harvest a berry and reset
			// growTime
	...
	private void setGrowthStage(boolean grow)
	{
		...
		// when the growth stage changes, we need to force the block to
		...
		if (prevGrowthStage != growthStage)
		{
			...
			worldObj.setBlockState(pos,
					state.withProperty(BlockCropPlot.AGE, Integer.valueOf(growthStage)), 2);
	...
	/**
	 * for each fertilizer slot: decreases the burn time, checks if
	 * burnTimeRemaining = 0 and tries to consume a new piece of fertilizer if
	 * one is available
	 *
	...
	/**
	 * Check if the plot is harvestable and there is sufficient space in the
	 * output slots
	 *
	...
	/**
	 * checks that there is an item to be harvested in one of the input slots
	 * and that there is room for the result in the output slots If desired,
	 * performs the berry harvest
	 *
	 * @param harvestBerry
	...
	// returns the growth result for the given stack. Returns null if the given
	// stack can not be grown
	public static ItemStack getGrowingResultForItem(ItemStack stack)
	{
		// TODO: Can't return a seed with only one output slot. Will have to
		// wait for a new design with more output slots.
		// Random rand = new Random();
		// if (rand.nextInt(100) == 0)
		// return stack;
		// else
		return ARKCraftSeed.getBerryForSeed(stack);
	}

	// returns the number of ticks the given item will grow. Returns 0 if the
	...
	{
		if (stack != null && stack.getItem() instanceof ARKCraftFeces)
		{
			return true;
		}
		else
	...
	{
		if (stack != null && stack.getItem() instanceof ARKCraftSeed)
		{
			return true;
		}
		else
