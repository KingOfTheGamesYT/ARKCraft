...
public abstract class GUIScrollable extends GUIArkContainer
		implements IGuiScrollable
{
	public static final ResourceLocation scrollButtonCreativeResource =
	...
	public static final int scrollButtonCreativeDisabledU =
			scrollButtonCreativeU + scrollButtonCreativeWidth;
	public static final int scrollButtonCreativeDisabledV =
			scrollButtonCreativeV;
			...
			int mouseX, int mouseY)
	{
		...
		this.drawTexturedModalRect(guiLeft + getScrollBarStartX(), y,
				canScroll() ? getScrollButtonU() : getScrollButtonDisabledU(),
				canScroll() ? getScrollButtonV() : getScrollButtonDisabledV(),
				getScrollButtonWidth(), getScrollButtonHeight());
	}
	...
	public void handleMouseInput() throws IOException
	{
		...
		if (scrollAmount != 0 && canScroll())
		{
			...
			scroll(scrollAmount);
			scrollPosition = ((ContainerScrollable) inventorySlots)
					.getRelativeScrollingOffset();
		}
	}
			...
			throws IOException
	{
		if (mouseButton == 0)
		{
			...
			{
				if (isPointInRegion(getScrollBarStartX(),
						(int) (getScrollBarStartY()
								+ getActualScrollPosition()),
						getScrollButtonWidth(), getScrollButtonHeight(), mouseX,
						mouseY))
				{
					scrollClicked = true;
				}
				if (isPointInRegion(getScrollBarStartX(), getScrollBarStartY(),
						getScrollButtonWidth(),
						getScrollBarEndY() + getScrollButtonHeight()
								- getScrollBarStartY(),
						mouseX, mouseY))
				{
	...
	{
		return (int) ((float) (getScrollBarEndY() - getScrollBarStartY() - 17)
				* getScrollPosition());
	}
	...
	private void adjustScrollFromMouseY(int mouseY)
	{
		...
		this.scrollPosition =
				((float) (mouseY - top) - 7.5F) / ((float) (bot - top) - 15.0F);
		this.scrollPosition =
				MathHelper.clamp_float(this.scrollPosition, 0.0F, 1.0F);
