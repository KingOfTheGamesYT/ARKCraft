...
public class PageResource extends Page
{
    ...
    public void draw(int guiLeft, int guiTop, int mouseX, int mouseY, SmallFontRenderer renderer, boolean canTranslate, GuiInfoBook book)
    {
        ...
        if (title != null) {
            ...
            renderer.drawSplitString(ChatFormatting.BOLD + "" + ChatFormatting.UNDERLINE + title, guiLeft - 8 + (book.guiWidth - renderer.getStringWidth(title)) / 2, guiTop + 5, 1000, 0);
        }
        ...
        if (information != null) {
            if (canTranslate) {
                information = I18n.format(information);
            }
            renderer.drawSplitString(information, guiLeft - 36 + (renderer.splitStringWidth(information, book.guiWidth)), guiTop + 40, book.guiWidth - 20, 0);
        }
    }
